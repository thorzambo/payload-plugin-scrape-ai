...
const SCRAPE_AI_REWRITES = [
  ...
  { source: '/ai/sitemap.xml', destination: '/api/scrape-ai/sitemap-xml' },
...
const SCRAPE_AI_HEADERS = [
  // Advertise AI content on EVERY page response
  {
    source: '/((?!api|_next|ai/).*)',
    headers: [
      { key: 'Link', value: '</llms.txt>; rel="ai-content"; type="text/markdown", </.well-known/ai-plugin.json>; rel="ai-plugin"; type="application/json"' },
      { key: 'X-AI-Content', value: '/llms.txt' },
      { key: 'X-AI-Discovery', value: '/.well-known/ai-plugin.json' },
    ],
  },
...
export function withScrapeAi(nextConfig: NextConfig = {}): NextConfig {
  ...
  return {
    ...
    async rewrites() {
      let existing: any = { beforeFiles: [], afterFiles: [], fallback: [] }
