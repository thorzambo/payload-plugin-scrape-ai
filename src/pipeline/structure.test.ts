...
describe('structureContent', () => {
  const baseParams = {
    ...
    collectionConfig: { slug: 'pages', fields: [] } as any,
  ...
  it('escapes special characters in YAML', () => {
    const result = structureContent({
      ...baseParams,
      doc: { ...baseParams.doc, title: 'Test "with" quotes\nand newlines' },
    })
  ...
  it('detects draft status', () => {
    const result = structureContent({
      ...baseParams,
      doc: { ...baseParams.doc, _status: 'draft' },
    })
  ...
  it('returns correct slug (dashes instead of slashes)', () => {
    const result = structureContent({
      ...baseParams,
      doc: { ...baseParams.doc, slug: 'blog/my-post' },
    })
