...
export interface ModelInfo {
  ...
  name: string
  contextWindow: number // max tokens the model can handle per request
...
export interface DocumentTokenEstimate {
  ...
  sourceDocId: string
  title: string
  contentTokens: number // tokens in the document markdown
  truncatedTokens: number // tokens after truncation (max 4000 chars for summary/entities, 6000 for chunks)
  calls: {
    summary: { inputTokens: number; outputTokens: number }
    entities: { inputTokens: number; outputTokens: number }
    chunks: { inputTokens: number; outputTokens: number }
  }
...
export function estimateDocumentTokens(
  ...
  return {
    ...
    totalInputTokens:
      (summaryInputTokens + summarySystemTokens) +
      (summaryInputTokens + entitiesSystemTokens) +
      (chunksInputTokens + chunksSystemTokens),
    totalOutputTokens:
      summaryOutputTokens + entitiesOutputTokens + Math.ceil(chunksOutputTokens),
  }
}
...
export interface JobEstimate {
  ...
  totals: {
    ...
    totalOutputTokens: number
    totalTokens: number
    maxSingleRequestTokens: number // largest single call input+output — model context window must exceed this
  }
  costEstimates: Array<{
    model: ModelInfo
    inputCost: number
    outputCost: number
    totalCost: number
    canHandle: boolean // context window is large enough
    recommended: boolean
    reason: string
  }>
  recommendation: {
...
export function estimateJob(
  documents: Array<{ title: string; markdown: string; sourceCollection: string; sourceDocId: string; hasAiMeta: boolean }>,
  preferredProvider?: 'openai' | 'anthropic',
): JobEstimate {
  const needsEnrichment = documents.filter((d) => !d.hasAiMeta && d.markdown)

  const perDocEstimates = needsEnrichment.map((doc) =>
    estimateDocumentTokens(doc.title, doc.markdown, doc.sourceCollection, doc.sourceDocId),
  )
  ...
  const costEstimates = models.map((model) => {
    const inputCost = (totalInputTokens / 1_000_000) * model.inputPricePerMTok
    const outputCost = (totalOutputTokens / 1_000_000) * model.outputPricePerMTok
  ...
  let recommendation: JobEstimate['recommendation'] = null
