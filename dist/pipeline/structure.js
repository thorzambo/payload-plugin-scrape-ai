 ...
 */ export function structureContent(params) {
    ...
    const children = allContent ? allContent.filter((c)=>c.sourceCollection === collectionSlug && c.slug !== urlSlug && c.parentSlug === urlSlug).map((c)=>c.slug) : [];
...
function buildRelatedSection(relatedSlugs, collectionSlug, siteUrl) {
    ...
    const links = relatedSlugs.map((slug)=>`- [${slug}](${siteUrl}/ai/${collectionSlug}/${slug}.md)`).join('\n');
